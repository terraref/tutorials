...
we want to calculate metric called growing degree days
first convert to just day, like did for traits
and convert air temp from kelvins to celsius
...
get lowest and highest temp for each day
first "subset" data by day column, takes all rows for each unique day together
then new column for daily min and max temps
...
then get mean of these in new column avg_temp
...
use that average temp and a chosen base temp to calculate gdd
gdd = accumulation of heat over time, only for temps above a base temperature

if mean of min and max daily temp is more than 10 degrees, 
then take mean and subtract 10

this gets amount of heat per day when exceeds threshold temp
then want cumulative across year, so add each onto the last
...
put them together based on day using join
full join, so every day for both dataframes combined
only using columns of interest
...
if there's no values for either cover or gdd, puts NA there
spotty coverage across days for both
remove any row that has NA in any column
