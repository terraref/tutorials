...
key <- read.table("/data/terraref/sites/genomics/raw_data/ril/gbs/Key_ril_terra",
                  header = TRUE)
key$LibraryPrepID <- seq(1:nrow(key))
key$Enzyme <- rep("ApeKI", times = nrow(key))
key$FullSampleName <- paste0(as.character(key$Sample), ":", 
                             as.character(key$LibraryPrepID))
names(key) <- c("Flowcell", "Lane", "Barcode", "DNASample", "LibraryPlate", 
                "Row", "Col", "LibraryPrepID", "Enzyme")
write.table(key, "/home/rstudio/tutorials/genomics/key_v5",
