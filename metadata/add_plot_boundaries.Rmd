...
# Adding plot boundaries to BETYdb sites using `sf` package
...
If you need to convert the coordinate reference system to another, you can use the `st_transform` function. In this example, we will be converting the EPSG from `32612` to `4326`.
...
## Write site boundaries to BETYdb using `st_write`

The `st_write` function can be used to write a new table to BETYdb. The new table will be named `s8_bounds` and will contain the contents of the `sf` object. This table will later be dropped after inserting new data into the `sites` table.
...
# Updating and inserting new records into BETYdb using `RPostgreSQL` package

The MAC Season 8 shapefiles did not inlude a `sitename` attribute. We will need to add this field to the `s8_bounds` table and update its values. The `dbSendStatement` function from the `RPostgreSQL` package will be used to make the updates.

SQL statement to add `sitename` field to `s8_bounds` table: 

`alter table s8_bounds add column sitename VARCHAR`

SQL statement to update `sitename` values:
...
## Add new records to BETYdb sites table using subset of s8_bounds
