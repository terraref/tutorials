...
The value of `eval_chk` will depend on the value you set for `commit`. This variable will determine whether chunks for checking uploaded data will be evaluated. 
...
# add_experiments
add_experiments <- function(experiments, users, dbcon, commit){
...
# add_sites
add_sites <- function(sites, dbcon, commit){
...
# add_treatments
add_treatments <- function(treatments, dbcon, commit){
...
# add_citations
add_citations <- function(citations, dbcon, commit){
...
add_row <- function(row, dbcon, tbl_name, tbl_fields, user_id = NULL, commit = FALSE){
  ...
  if(tbl_name == 'experiments'){ # user id needed to insert data into experiments
...
Your metadata will be read in using google sheets. Each sheet will be read in directly as a R object.
...
First you need to make sure that all species present in the cultivars sheet has been added to the BETYdb species table. This is needed since the cultivars table references the species table for specie_id.
...
Since this file will be used for testing, new species added by the `get_new_cultivars` function will be removed unless you choose to upload your data. The following chunk will not be run unless you set the value of `commit` to be `TRUE` on line 40.
