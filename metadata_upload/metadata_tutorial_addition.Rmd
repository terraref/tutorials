...
The `Metadata Upload Tutorial Part 2: Associations` tutorial should be viewed after completing this tutorial and when all new metadata has been added (see below for more details). The second tutorial will cover how to associate the metadata that you add.
...
To upload your data to the test database, you will need to make an edit to this markdown file. 

`commit = TRUE` needs to be set in calls to `addRow`.

When you are done testing and are ready to upload your data to the test database, change the value of commit from `FALSE` to `TRUE` in the following chunk:
...
# addSpecie
# species_to_add should be character vector of specie names that need to be added to db
addSpecie <- function(species_to_add,
                      commit = FALSE){ 
...
addSpecie(commit = commit,
          specie_to_add  = spp_to_add)
