...
envlog_file <- "~/Globus/envlog_netcdf/2017-08-20/2017-08-20_03-11-57_environmentlogger.nc"
#envlog_file <- "~/Globus/envlog_netcdf/2017-08-20/envlog_netcdf_L1_ua-mac_2017-08-20.nc"
...
#metadata <- as.data.frame(envlog.nc$var)
metadata <- jsonlite::fromJSON('~/dev/data-paper/metadata/envlog_netcdf_metadata.json',  flatten = TRUE)
...
#ncmetadata <- ncmeta::nc_meta(envlog.nc)

variables <- names(envlog.nc$var)

x <- data.frame(variable = character(length = length(envlog.nc$var)))
x$variable <- envlog.nc$var
for(v in variables){
  

}
nc_metadata$variable %>% 
  select(name, units, ndims) %>% 
